...
class Trigon:
    def __init__(self, *args):
        """
        Получает неизвестное количество атрибутов и проверяет по условиям
        :param args:
        """
        self.next = args
        if len(self.next) != 3:
            raise IndexError(f'Передано {len(self.next)} аргументов, а ожидается 3')
        a = self.next[0]
        b = self.next[1]
        c = self.next[2]
        if type(a) != int or type(b) != int or type(c) != int:  # cторона передана не числом
            raise TypeError('Стороны должны быть числами')
        elif a <= 0 or b <= 0 or c <= 0:   # сторона передана нулем или отрицательным числом
            raise ValueError('Стороны должны быть положительными')
        elif not a + b > c and b + c > a and c + a > b:  # не соблюдается неравество треугольника
